...
import { ITemplateModel, ILateDeliveryAndPenaltyRequest } from './generated/org.accordproject.latedeliveryandpenaltycurrencyconversion@0.3.0';
...
describe('LateDeliveryAndPenaltyCurrencyConversionLogic', () => {
    ...
    beforeEach(() => {
        ...
        model = {
            $class: 'org.accordproject.latedeliveryandpenaltycurrencyconversion@0.3.0.TemplateModel',
            $identifier: 'test-id',
            clauseId: 'test-id',
            buyer: 'Alice',
            seller: 'Bob',
    ...
    describe('trigger', () => {
        it('should return penalty=0 and buyerMayTerminate=true when force majeure applies', async () => {
            const request: ILateDeliveryAndPenaltyRequest = {
                $class: 'org.accordproject.latedeliveryandpenaltycurrencyconversion@0.3.0.LateDeliveryAndPenaltyRequest',
                $timestamp: new Date(),
                forceMajeure: true,
                agreedDelivery: pastDate,
                goodsValue: 1000,
                currencyConversion: {
                    $class: 'org.accordproject.latedeliveryandpenaltycurrencyconversion@0.3.0.CurrencyConversion',
                    from: 'USD',
                    to: 'EUR',
                    rate: 1.1,
                },
        ...
        it('should emit a PaymentObligationEvent with seller and buyer in description', async () => {
            ...
            expect(result.events).toHaveLength(1);
            const event = result.events[0] as any;
            expect(event.$class).toBe('org.accordproject.latedeliveryandpenaltycurrencyconversion@0.3.0.PaymentObligationEvent');
