...
import { ITemplateModel, ILateDeliveryAndPenaltyRequest } from './generated/org.accordproject.latedeliveryandpenaltycurrencyconversion@0.2.0';
    ...
    describe('trigger', () => {
        it('should return penalty=0 and buyerMayTerminate=true when force majeure applies', async () => {
            const request: ILateDeliveryAndPenaltyRequest = {
                $class: 'org.accordproject.latedeliveryandpenaltycurrencyconversion@0.2.0.LateDeliveryAndPenaltyRequest',
                $timestamp: new Date(),
                forceMajeure: true,
        ...
        it('should emit a PaymentObligationEvent with seller and buyer in description', async () => {
            ...
            expect(result.events).toHaveLength(1);
            const event = result.events[0] as any;
            expect(event.$class).toBe('org.accordproject.latedeliveryandpenaltycurrencyconversion@0.2.0.PaymentObligationEvent');
