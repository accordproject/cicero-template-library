import { ITemplateModel, ILateDeliveryAndPenaltyRequest, ILateDeliveryAndPenaltyResponse, IPaymentObligationEvent } from './generated/org.accordproject.latedeliveryandpenaltycurrencyconversion@0.3.0';
...
class LateDeliveryAndPenaltyCurrencyConversionLogic extends TemplateLogic<ITemplateModel> {
    async trigger(data: ITemplateModel, request: ILateDeliveryAndPenaltyRequest): Promise<LateDeliveryAndPenaltyResult> {
        ...
        if (data.forceMajeure && request.forceMajeure) {
            return {
                result: {
                    $class: 'org.accordproject.latedeliveryandpenaltycurrencyconversion@0.3.0.LateDeliveryAndPenaltyResponse',
                    $timestamp: now,
                    penalty: 0,
        ...
        const event: IPaymentObligationEvent = {
            $class: 'org.accordproject.latedeliveryandpenaltycurrencyconversion@0.3.0.PaymentObligationEvent',
            $timestamp: now,
            amount: capped,
