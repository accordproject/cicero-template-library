import { ITemplateModel, ILateDeliveryAndPenaltyRequest, ILateDeliveryAndPenaltyResponse, IPaymentObligationEvent } from './generated/org.accordproject.latedeliveryandpenaltyoptionalthis@0.2.0';
...
class LateDeliveryAndPenaltyOptionalThisLogic extends TemplateLogic<ITemplateModel> {
    async trigger(data: ITemplateModel, request: ILateDeliveryAndPenaltyRequest): Promise<LateDeliveryAndPenaltyResult> {
        ...
        if (data.forceMajeure && request.forceMajeure) {
            return {
                result: {
                    ...
                    $timestamp: now,
                    penalty: monetary(0, currencyCode),
        ...
        const event: IPaymentObligationEvent = {
            $class: 'org.accordproject.latedeliveryandpenaltyoptionalthis@0.2.0.PaymentObligationEvent',
            $timestamp: now,
            amount: cappedAmount,
