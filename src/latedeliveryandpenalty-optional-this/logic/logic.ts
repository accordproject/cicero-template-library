import { ITemplateModel, ILateDeliveryAndPenaltyRequest, ILateDeliveryAndPenaltyResponse, IPaymentObligationEvent } from './generated/org.accordproject.latedeliveryandpenaltyoptionalthis@0.3.0';
...
class LateDeliveryAndPenaltyOptionalThisLogic extends TemplateLogic<ITemplateModel> {
    async trigger(data: ITemplateModel, request: ILateDeliveryAndPenaltyRequest): Promise<LateDeliveryAndPenaltyResult> {
        ...
        if (data.forceMajeure != null && request.forceMajeure != null) {
            return {
                result: {
                    $class: 'org.accordproject.latedeliveryandpenaltyoptionalthis@0.3.0.LateDeliveryAndPenaltyResponse',
                    $timestamp: now,
                    penalty: 0,
        ...
        const event: IPaymentObligationEvent = {
            $class: 'org.accordproject.latedeliveryandpenaltyoptionalthis@0.3.0.PaymentObligationEvent',
            $timestamp: now,
            amount: capped,
            currencyCode: 'USD',
...
export default LateDeliveryAndPenaltyOptionalThisLogic;
