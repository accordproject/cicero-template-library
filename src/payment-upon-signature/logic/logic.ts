    ...
    async trigger(
        ...
        if (requestClass.endsWith('.ContractSigned')) {
            ...
            const event: IPaymentObligationEvent = {
                $class: 'io.clause.paymentuponssignature@0.1.0.PaymentObligationEvent',
                $timestamp: now,
                amount: data.amount,
            ...
            return {
                result: { $class: 'io.clause.paymentuponssignature@0.1.0.ContractSignedResponse', $timestamp: now },
                state: { $class: 'io.clause.paymentuponssignature@0.1.0.PaymentUponSignatureState', $identifier: state.$identifier, status: 'OBLIGATION_EMITTED' },
                events: [event],
            };
            ...
            return {
                result: { $class: 'io.clause.paymentuponssignature@0.1.0.PaymentReceivedResponse', $timestamp: now },
                state: { $class: 'io.clause.paymentuponssignature@0.1.0.PaymentUponSignatureState', $identifier: state.$identifier, status: 'COMPLETED' },
                events: [],
            };
        } else {
