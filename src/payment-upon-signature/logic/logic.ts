...
class PaymentUponSignatureLogic extends TemplateLogic<ITemplateModel, IPaymentUponSignatureState> {
    ...
    async trigger(
        ...
        if (requestClass.endsWith('.ContractSigned')) {
            ...
            return {
                result: { $class: 'org.accordproject.paymentuponssignature@0.2.0.ContractSignedResponse', $timestamp: now },
                state: { $class: 'org.accordproject.paymentuponssignature@0.2.0.PaymentUponSignatureState', $identifier: state.$identifier, status: 'OBLIGATION_EMITTED' },
                events: [event],
            };
            ...
            return {
                result: { $class: 'org.accordproject.paymentuponssignature@0.2.0.PaymentReceivedResponse', $timestamp: now },
                state: { $class: 'org.accordproject.paymentuponssignature@0.2.0.PaymentUponSignatureState', $identifier: state.$identifier, status: 'COMPLETED' },
                events: [],
            };
        } else {
