...
} from './generated/org.accordproject.paymentuponssignature@0.3.0';
...
class PaymentUponSignatureLogic extends TemplateLogic<ITemplateModel, IPaymentUponSignatureState> {
    ...
    async init(data: ITemplateModel): Promise<InitResponse<IPaymentUponSignatureState>> {
        return {
            state: {
                $class: 'org.accordproject.paymentuponssignature@0.3.0.PaymentUponSignatureState',
    ...
    async trigger(
        ...
        if (requestClass.endsWith('.ContractSigned')) {
            ...
            const event: IPaymentObligationEvent = {
                $class: 'org.accordproject.paymentuponssignature@0.3.0.PaymentObligationEvent',
                $timestamp: now,
                amount: data.amount,
            ...
            return {
                result: { $class: 'org.accordproject.paymentuponssignature@0.3.0.ContractSignedResponse', $timestamp: now },
                state: { $class: 'org.accordproject.paymentuponssignature@0.3.0.PaymentUponSignatureState', $identifier: state.$identifier, status: 'OBLIGATION_EMITTED' },
                events: [event],
            };
            ...
            return {
                result: { $class: 'org.accordproject.paymentuponssignature@0.3.0.PaymentReceivedResponse', $timestamp: now },
                state: { $class: 'org.accordproject.paymentuponssignature@0.3.0.PaymentUponSignatureState', $identifier: state.$identifier, status: 'COMPLETED' },
                events: [],
            };
        } else {
