import { ITemplateModel, IVolumeDiscountRequest, IVolumeDiscountResponse } from './generated/org.accordproject.volumediscount@0.3.0';
...
class VolumeDiscountLogic extends TemplateLogic<ITemplateModel> {
    async trigger(data: ITemplateModel, request: IVolumeDiscountRequest): Promise<VolumeDiscountResult> {
        ...
        return {
            result: {
                $class: 'org.accordproject.volumediscount@0.3.0.VolumeDiscountResponse',
                $timestamp: new Date(),
                discountRate,
            },
        };
