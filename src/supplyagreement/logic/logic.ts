...
class SupplyAgreementLogic extends TemplateLogic<ITemplateModel, IAgreementState> {
    ...
    async trigger(
        ...
        if (requestClass === 'org.accordproject.supplyagreement@0.3.0.ForecastRequest') {
            return this.demandForecast(data, request as IForecastRequest, state);
        } else if (requestClass === 'org.accordproject.supplyagreement@0.3.0.PurchaseRequest') {
            return this.purchase(data, request as IPurchaseRequest, state);
        } else if (requestClass === 'org.accordproject.supplyagreement@0.3.0.DeliveryRequest') {
    ...
    private async demandForecast(
        ...
        const event: IPurchaseObligationEvent = {
            $class: 'org.accordproject.supplyagreement@0.3.0.PurchaseObligationEvent',
            $timestamp: now,
            party: data.buyer,
            requiredPurchase,
            year,
        ...
        return {
            result: {
                $class: 'org.accordproject.supplyagreement@0.3.0.ForecastResponse',
                $timestamp: now,
            },
            state: newState,
            events: [event],
    ...
    private async purchase(
        ...
        const orderItems: IOrderItem[] = request.purchaseOrder.products.map(p => ({
            $class: 'org.accordproject.supplyagreement@0.3.0.OrderItem',
            partNumber: p.name,
            quantity: p.quantity,
        }));
        ...
        const event: IDeliveryObligationEvent = {
            $class: 'org.accordproject.supplyagreement@0.3.0.DeliveryObligationEvent',
            $timestamp: now,
            party: data.supplier,
            expectedDelivery: request.purchaseOrder.deliveryDate,
            deliverables: orderItems,
        ...
        return {
            result: {
                $class: 'org.accordproject.supplyagreement@0.3.0.PurchaseResponse',
                $timestamp: now,
            },
            state: newState,
            events: [event],
    ...
    private async delivery(
        ...
        const paymentObligation = {
            $class: 'org.accordproject.supplyagreement@0.3.0.PaymentObligationData',
            party: data.buyer,
            amount,
        };
        ...
        const event: IPaymentObligationEvent = {
            $class: 'org.accordproject.supplyagreement@0.3.0.PaymentObligationEvent',
            $timestamp: now,
            party: data.buyer,
            amount,
        };
        ...
        return {
            result: {
                $class: 'org.accordproject.supplyagreement@0.3.0.DeliveryResponse',
                $timestamp: now,
            },
            state: newState,
            events: [event],
