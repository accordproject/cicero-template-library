...
import { ITemplateModel, IEmptyRequest } from './generated/org.accordproject.emptycontract@0.1.0';
...
describe('EmptyContractLogic', () => {
    ...
    beforeEach(() => {
        ...
        model = {
            $class: 'org.accordproject.emptycontract@0.1.0.TemplateModel',
            $identifier: 'test-id',
            clauseId: 'test-id',
        };
    });
    ...
    describe('trigger', () => {
        it('should return an empty response', async () => {
            const request: IEmptyRequest = {
                $class: 'org.accordproject.emptycontract@0.1.0.EmptyRequest',
                $timestamp: new Date(),
            };
            const response = await logic.trigger(model, request);
            expect(response.result.$class).toBe('org.accordproject.emptycontract@0.1.0.EmptyResponse');
