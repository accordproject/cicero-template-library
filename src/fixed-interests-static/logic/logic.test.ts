...
import { ITemplateModel, IFixedInterestsStaticRequest } from './generated/org.accordproject.fixedinterestsstatic@0.2.0';
...
describe('FixedInterestsStaticLogic', () => {
    ...
    beforeEach(() => {
        ...
        model = {
            $class: 'org.accordproject.fixedinterestsstatic@0.2.0.TemplateModel',
            $identifier: 'test-id',
            clauseId: 'test-id',
            loanAmount: 100000.0,
            currencyCode: 'USD',
    ...
    describe('trigger', () => {
        it('should return a response with output mentioning the loan amount', async () => {
            const request: IFixedInterestsStaticRequest = {
                $class: 'org.accordproject.fixedinterestsstatic@0.2.0.FixedInterestsStaticRequest',
                $timestamp: new Date(),
                input: 'test'
            };
            const result = await logic.trigger(model, request);

            expect(result.result).toHaveProperty('$class', 'org.accordproject.fixedinterestsstatic@0.2.0.FixedInterestsStaticResponse');
