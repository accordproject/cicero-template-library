    ...
    async trigger(
        ...
        if (now >= new Date(data.dueDate)) {
            const lateEvent: IPaymentObligationEvent = {
                $class: 'org.accordproject.supplyagreementperishablegoods@0.3.0.PaymentObligationEvent',
                $timestamp: now,
                grower: data.grower,
            ...
            return {
                result: {
                    $class: 'org.accordproject.supplyagreementperishablegoods@0.3.0.PriceCalculation',
                    $timestamp: now,
                    totalPrice: 0.0,
                    penalty: 0.0,
                    currencyCode: data.currencyCode,
