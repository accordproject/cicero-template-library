...
import {
    ITemplateModel,
    IShipmentReceived,
} from './generated/org.accordproject.supplyagreementperishablegoods@0.3.0';
...
describe('SupplyAgreementPerishableGoodsLogic', () => {
    ...
    beforeEach(() => {
        ...
        model = {
            $class: 'org.accordproject.supplyagreementperishablegoods@0.3.0.TemplateModel',
    ...
    const makeRequest = (unitCount: number, centigrade: number, humidity: number): IShipmentReceived => ({
        $class: 'org.accordproject.supplyagreementperishablegoods@0.3.0.ShipmentReceived',
        $identifier: 'req-1',
        $timestamp: new Date(),
        unitCount,
        shipment: {
            $class: 'org.accordproject.supplyagreementperishablegoods@0.3.0.Shipment',
            shipmentId: 'SHIP_001',
            status: 'IN_TRANSIT',
            sensorReadings: [
                {
                    $class: 'org.accordproject.supplyagreementperishablegoods@0.3.0.SensorReading',
                    centigrade,
                    humidity,
                }
            ],
    ...
    describe('trigger - normal case (within temperature and humidity range)', () => {
        it('should return full payout with no penalty', async () => {
            ...
            expect(result.result.$class).toBe('org.accordproject.supplyagreementperishablegoods@0.3.0.PriceCalculation');
            ...
            expect(result.events[0].$class).toBe('org.accordproject.supplyagreementperishablegoods@0.3.0.PaymentObligationEvent');
        });
    });
