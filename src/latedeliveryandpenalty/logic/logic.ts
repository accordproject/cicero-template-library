import { ITemplateModel, ILateDeliveryAndPenaltyRequest, ILateDeliveryAndPenaltyResponse, IPaymentObligationEvent } from './generated/org.accordproject.latedeliveryandpenalty@0.1.0';
...
class LateDeliveryAndPenaltyLogic extends TemplateLogic<ITemplateModel> {
    async trigger(data: ITemplateModel, request: ILateDeliveryAndPenaltyRequest): Promise<LateDeliveryAndPenaltyResult> {
        ...
        if (data.forceMajeure && request.forceMajeure) {
            return {
                result: {
                    $class: 'org.accordproject.latedeliveryandpenalty@0.1.0.LateDeliveryAndPenaltyResponse',
                    $timestamp: now,
                    penalty: 0,
        ...
        const event: IPaymentObligationEvent = {
            $class: 'org.accordproject.latedeliveryandpenalty@0.1.0.PaymentObligationEvent',
            $timestamp: now,
            amount: capped,
            currencyCode: 'USD',
