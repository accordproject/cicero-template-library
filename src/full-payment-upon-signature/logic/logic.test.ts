...
describe('FullPaymentUponSignatureLogic', () => {
    ...
    beforeEach(() => {
        ...
        model = {
            $class: 'org.accordproject.fullpaymentupondsignature@0.3.0.TemplateModel',
            $identifier: 'test-id',
            clauseId: 'test-id',
            buyer: 'Alice',
            seller: 'Bob',
    ...
    describe('init', () => {
        it('should initialize with INITIALIZED status', async () => {
            ...
            expect((result.state as any).$class).toBe('org.accordproject.fullpaymentupondsignature@0.3.0.FullPaymentUponSignatureState');
        });
    });
    ...
    describe('trigger - ContractSigned', () => {
        it('should emit payment obligation and move to OBLIGATION_EMITTED', async () => {
            const request = {
                $class: 'org.accordproject.fullpaymentupondsignature@0.3.0.ContractSigned',
                $timestamp: new Date(),
            };
            ...
            expect(event.$class).toBe('org.accordproject.fullpaymentupondsignature@0.3.0.PaymentObligationEvent');
    ...
    describe('trigger - PaymentReceived', () => {
        it('should complete the contract when payment is received', async () => {
            const request = {
                $class: 'org.accordproject.fullpaymentupondsignature@0.3.0.PaymentReceived',
                $timestamp: new Date(),
            };
            const state = { ...initializedState, status: 'OBLIGATION_EMITTED' };
