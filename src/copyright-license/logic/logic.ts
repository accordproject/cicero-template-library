import { ITemplateModel, IPaymentRequest, IPayOut, IPaymentObligationEvent } from "./generated/org.accordproject.copyrightlicense@0.3.0";
...
class CopyrightLicenseLogic extends TemplateLogic<ITemplateModel> {
    async trigger(data: ITemplateModel, request: IPaymentRequest): Promise<CopyrightLicenseResponse> {
        const event: IPaymentObligationEvent = {
            $class: 'org.accordproject.copyrightlicense@0.3.0.PaymentObligationEvent',
            $timestamp: new Date(),
        ...
        return {
            result: {
                $class: 'org.accordproject.copyrightlicense@0.3.0.PayOut',
                $timestamp: new Date(),
                amount: data.paymentClause.amount,
                currencyCode: data.paymentClause.currencyCode
            },
