...
describe('FullPaymentUponDemandLogic', () => {
    ...
    describe('init', () => {
        it('should initialize with INITIALIZED status', async () => {
            ...
            expect((result.state as any).$class).toBe('org.accordproject.fullpaymentupondemand@0.1.0.FullPaymentUponDemandState');
        });
    });
    ...
    describe('trigger - PaymentDemand', () => {
        it('should emit a payment obligation event and move to OBLIGATION_EMITTED', async () => {
            const request = {
                $class: 'org.accordproject.fullpaymentupondemand@0.1.0.PaymentDemand',
                $timestamp: new Date(),
            };
            ...
            expect(event.$class).toBe('org.accordproject.fullpaymentupondemand@0.1.0.PaymentObligationEvent');
    ...
    describe('trigger - PaymentReceived', () => {
        it('should complete the contract when payment is received', async () => {
            const request = {
                $class: 'org.accordproject.fullpaymentupondemand@0.1.0.PaymentReceived',
                $timestamp: new Date(),
            };
            const state = { ...initializedState, status: 'OBLIGATION_EMITTED' };
