    ...
    async trigger(
        ...
        if (requestClass.endsWith('.PaymentDemand')) {
            ...
            const event: IPaymentObligationEvent = {
                $class: 'io.clause.fullpaymentupondemand@0.1.0.PaymentObligationEvent',
                $timestamp: now,
                amount: data.amount,
            ...
            return {
                result: { $class: 'io.clause.fullpaymentupondemand@0.1.0.PaymentDemandResponse', $timestamp: now },
                state: { $class: 'io.clause.fullpaymentupondemand@0.1.0.FullPaymentUponDemandState', $identifier: state.$identifier, status: 'OBLIGATION_EMITTED' },
                events: [event],
            };
            ...
            return {
                result: { $class: 'io.clause.fullpaymentupondemand@0.1.0.PaymentReceivedResponse', $timestamp: now },
                state: { $class: 'io.clause.fullpaymentupondemand@0.1.0.FullPaymentUponDemandState', $identifier: state.$identifier, status: 'COMPLETED' },
                events: [],
            };
        } else {
