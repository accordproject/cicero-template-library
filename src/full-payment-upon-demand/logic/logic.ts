...
} from './generated/org.accordproject.fullpaymentupondemand@0.3.0';
...
class FullPaymentUponDemandLogic extends TemplateLogic<ITemplateModel, IFullPaymentUponDemandState> {
    ...
    async init(data: ITemplateModel): Promise<InitResponse<IFullPaymentUponDemandState>> {
        return {
            state: {
                $class: 'org.accordproject.fullpaymentupondemand@0.3.0.FullPaymentUponDemandState',
    ...
    async trigger(
        ...
        if (requestClass.endsWith('.PaymentDemand')) {
            ...
            const event: IPaymentObligationEvent = {
                $class: 'org.accordproject.fullpaymentupondemand@0.3.0.PaymentObligationEvent',
                $timestamp: now,
                amount: data.amount,
            ...
            return {
                result: { $class: 'org.accordproject.fullpaymentupondemand@0.3.0.PaymentDemandResponse', $timestamp: now },
                state: { $class: 'org.accordproject.fullpaymentupondemand@0.3.0.FullPaymentUponDemandState', $identifier: state.$identifier, status: 'OBLIGATION_EMITTED' },
                events: [event],
            };
            ...
            return {
                result: { $class: 'org.accordproject.fullpaymentupondemand@0.3.0.PaymentReceivedResponse', $timestamp: now },
                state: { $class: 'org.accordproject.fullpaymentupondemand@0.3.0.FullPaymentUponDemandState', $identifier: state.$identifier, status: 'COMPLETED' },
                events: [],
            };
        } else {
