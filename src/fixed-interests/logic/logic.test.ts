...
import { ITemplateModel, IFixedInterestsRequest } from './generated/org.accordproject.fixedinterests@0.3.0';
...
describe('FixedInterestsLogic', () => {
    ...
    beforeEach(() => {
        ...
        model = {
            $class: 'org.accordproject.fixedinterests@0.3.0.TemplateModel',
            $identifier: 'test-id',
            clauseId: 'test-id',
            loanAmount: 100000.0,
            currencyCode: 'USD',
    ...
    describe('trigger', () => {
        it('should return a response with output mentioning the loan amount', async () => {
            const request: IFixedInterestsRequest = {
                $class: 'org.accordproject.fixedinterests@0.3.0.FixedInterestsRequest',
                $timestamp: new Date(),
                input: 'test'
            };
            const result = await logic.trigger(model, request);

            expect(result.result).toHaveProperty('$class', 'org.accordproject.fixedinterests@0.3.0.FixedInterestsResponse');
