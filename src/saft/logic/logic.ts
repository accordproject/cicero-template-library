import { ILaunch, ITerminate, IPayout, ITemplateModel } from "./generated/org.accordproject.saft@0.3.0";
...
class SaftLogic extends TemplateLogic<ITemplateModel> {
    async trigger(data: ITemplateModel, request: SaftRequest): Promise<SaftResponse> {
        if (request.$class === 'org.accordproject.saft@0.3.0.Launch') {
            return this.onLaunch(data, request as ILaunch);
        } else if (request.$class === 'org.accordproject.saft@0.3.0.Terminate') {
    ...
    private async onLaunch(data: ITemplateModel, request: ILaunch): Promise<SaftResponse> {
        return {
            result: {
                ...
                $timestamp: new Date(),
                $class: 'org.accordproject.saft@0.3.0.Payout'
            }
        };
    }
