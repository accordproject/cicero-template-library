...
import { ITemplateModel, ILaunch, ITerminate } from './generated/org.accordproject.saft@0.3.0';
...
describe('SaftLogic', () => {
    ...
    beforeEach(() => {
        logic = new SaftLogic();
        model = {
            $class: 'org.accordproject.saft@0.3.0.TemplateModel',
            $identifier: 'test-id',
            clauseId: 'test-id',
            token: 'TestToken',
    ...
    describe('trigger - onLaunch', () => {
        it('should pay out 100 tokens to purchaser on network launch', async () => {
            const request: ILaunch = {
                $class: 'org.accordproject.saft@0.3.0.Launch',
                $timestamp: new Date(),
                exchangeRate: 123
            };
            ...
            expect(result.result).toHaveProperty('$class', 'org.accordproject.saft@0.3.0.Payout');
    ...
    describe('trigger - onTerminate', () => {
        it('should pay out 9 tokens to purchaser on network termination', async () => {
            const request: ITerminate = {
                $class: 'org.accordproject.saft@0.3.0.Terminate',
                $timestamp: new Date(),
    ...
    describe('trigger - unknown request', () => {
        it('should throw for unknown request type', async () => {
            const request = {
                $class: 'org.accordproject.saft@0.3.0.Unknown',
                $timestamp: new Date()
            };
