...
} from "./generated/org.accordproject.docusignpofailure@0.3.0";
...
class PurchaseOrderFailureLogic extends TemplateLogic<ITemplateModel, IPurchaseOrderFailureState> {
    ...
    async init(data: ITemplateModel): Promise<InitResponse<IPurchaseOrderFailureState>> {
        return {
            state: {
                $class: 'org.accordproject.docusignpofailure@0.3.0.PurchaseOrderFailureState',
    ...
    async trigger(
        ...
        if (now <= deliveryMs + lateOneMs) {
            return {
                result: {
                    $class: 'org.accordproject.docusignpofailure@0.3.0.PurchaseOrderFailureResponse',
                    $timestamp: new Date(),
                    penaltyAmount: 0.0,
                    currencyCode
                },
        ...
        const event: IPurchaseOrderPaymentEvent = {
            $class: 'org.accordproject.docusignpofailure@0.3.0.PurchaseOrderPaymentEvent',
            $timestamp: new Date(),
            penaltyAmount,
            currencyCode,
