...
import { ITemplateModel, ILateDeliveryAndPenaltyRequest } from './generated/org.accordproject.latedeliveryandpenaltyelse@0.3.0';
...
describe('LateDeliveryAndPenaltyElseLogic', () => {
    ...
    beforeEach(() => {
        logic = new LateDeliveryAndPenaltyElseLogic();
        pastDate = new Date();
        pastDate.setDate(pastDate.getDate() - 14);
        model = {
            $class: 'org.accordproject.latedeliveryandpenaltyelse@0.3.0.TemplateModel',
            $identifier: 'test-id',
            clauseId: 'test-id',
            buyer: 'Alice',
            seller: 'Bob',
    ...
    describe('trigger', () => {
        it('should return penalty=0 and buyerMayTerminate=true when force majeure applies to both', async () => {
            const request: ILateDeliveryAndPenaltyRequest = {
                $class: 'org.accordproject.latedeliveryandpenaltyelse@0.3.0.LateDeliveryAndPenaltyRequest',
                $timestamp: new Date(),
                forceMajeure: true,
