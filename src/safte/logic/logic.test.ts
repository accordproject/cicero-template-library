...
import { ITemplateModel, ITokenSale, IEquityFinancing, IDissolutionEvent } from './generated/org.accordproject.safte@0.3.0';
...
describe('SafteLogic', () => {
    ...
    beforeEach(() => {
        ...
        model = {
            $class: 'org.accordproject.safte@0.3.0.TemplateModel',
            $identifier: 'test-id',
            clauseId: 'test-id',
            companyName: 'ACME',
    ...
    describe('trigger - tokenSale', () => {
        it('should compute token share on token sale', async () => {
            const request: ITokenSale = {
                $class: 'org.accordproject.safte@0.3.0.TokenSale',
                $timestamp: new Date(),
                tokenPrice: 1.23
            };
            ...
            expect(result.result).toHaveProperty('$class', 'org.accordproject.safte@0.3.0.TokenShare');
    ...
    describe('trigger - equityFinancing', () => {
        it('should compute equity share on equity financing', async () => {
            const request: IEquityFinancing = {
                $class: 'org.accordproject.safte@0.3.0.EquityFinancing',
                $timestamp: new Date(),
                sharePrice: 3.00
            };
            ...
            expect(result.result).toHaveProperty('$class', 'org.accordproject.safte@0.3.0.EquityShare');
            expect(result.result).toHaveProperty('$timestamp');
            // discountRate = (100 - 7) / 100 = 0.93
    ...
    describe('trigger - dissolutionEvent', () => {
        it('should refund the purchase amount on dissolution', async () => {
            const request: IDissolutionEvent = {
                $class: 'org.accordproject.safte@0.3.0.DissolutionEvent',
                $timestamp: new Date(),
                cause: 'Went Shopping'
            };
            ...
            expect(result.result).toHaveProperty('$class', 'org.accordproject.safte@0.3.0.PayOut');
            expect(result.result).toHaveProperty('$timestamp');
    ...
    describe('trigger - unknown request', () => {
        it('should throw for unknown request type', async () => {
            const request = {
                $class: 'org.accordproject.safte@0.3.0.Unknown',
                $timestamp: new Date()
            };
