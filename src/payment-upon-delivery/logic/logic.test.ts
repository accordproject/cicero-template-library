...
import { ITemplateModel, IDeliveryAcceptedRequest } from './generated/org.accordproject.paymentupondelivery@0.3.0';
...
describe('PaymentUponDeliveryLogic', () => {
    ...
    beforeEach(() => {
        ...
        model = {
            $class: 'org.accordproject.paymentupondelivery@0.3.0.TemplateModel',
            $identifier: 'test-id',
            clauseId: 'test-id',
            buyer: 'Alice',
            seller: 'Bob',
    ...
    describe('trigger', () => {
        it('should return total amount as sum of costOfGoods and deliveryFee', async () => {
            const request: IDeliveryAcceptedRequest = {
                $class: 'org.accordproject.paymentupondelivery@0.3.0.DeliveryAcceptedRequest',
                $timestamp: new Date(),
            };
            const result = await logic.trigger(model, request);
            expect(result.result.totalAmount).toBe(550);
            expect(result.result.currencyCode).toBe('USD');
            expect(result.result.$class).toBe('org.accordproject.paymentupondelivery@0.3.0.DeliveryAcceptedResponse');
        });
        ...
        it('should emit a PaymentObligationEvent', async () => {
            ...
            expect(event.$class).toBe('org.accordproject.paymentupondelivery@0.3.0.PaymentObligationEvent');
