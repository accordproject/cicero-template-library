...
describe('Logic', () => {
    ...
    describe('#PaymentUponDelivery', async function() {

        it('should produce correct payment in USD', async function() {
            const request = {};
            request.$class = 'org.accordproject.payment.upondelivery.DeliveryAcceptedRequest';

            const result = await engine.execute(clause, request, state);
            result.should.not.be.null;
            result.emit[0].$class.should.equal('org.accordproject.cicero.runtime.PaymentObligation');
            result.emit[0].amount.doubleValue.should.equal(11.98);
