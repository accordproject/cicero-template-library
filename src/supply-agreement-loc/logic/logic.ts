import {
    ...
    ICheckContract,
    IDeliveryResponse,
} from './generated/org.accordproject.supplyagreementloc@0.3.0';
...
class SupplyAgreementLocLogic extends TemplateLogic<ITemplateModel, ISupplyAgreementState> {
    ...
    async init(data: ITemplateModel): Promise<InitResponse<ISupplyAgreementState>> {
        return {
            state: {
                $class: 'org.accordproject.supplyagreementloc@0.3.0.SupplyAgreementState',
    ...
    async trigger(
        ...
        if (requestClass === 'org.accordproject.supplyagreementloc@0.3.0.SensorReading') {
            const reading = request as ISensorReading;
            const readingData: ISensorReadingData = {
                $class: 'org.accordproject.supplyagreementloc@0.3.0.SensorReadingData',
            ...
            return {
                result: {
                    $class: 'org.accordproject.supplyagreementloc@0.3.0.DeliveryResponse',
                    $timestamp: now,
                    $identifier: state.$identifier,
        ...
        } else if (requestClass === 'org.accordproject.supplyagreementloc@0.3.0.CheckContract') {
