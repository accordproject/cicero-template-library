import { ITemplateModel, ILateInvoiceRequest, ILateInvoiceResponse, IPaymentObligationEvent } from './generated/org.accordproject.lateinvoicewithpayment@0.3.0';
...
class LateInvoiceWithPaymentLogic extends TemplateLogic<ITemplateModel> {
    ...
    async trigger(data: ITemplateModel, request: ILateInvoiceRequest): Promise<LateInvoicePaymentResponse> {
        ...
        if (now >= terminationDate) {
            return {
                result: {
                    $class: 'org.accordproject.lateinvoicewithpayment@0.3.0.LateInvoiceResponse',
                    $timestamp: now,
        ...
        const event: IPaymentObligationEvent = {
            $class: 'org.accordproject.lateinvoicewithpayment@0.3.0.PaymentObligationEvent',
            $timestamp: now,
