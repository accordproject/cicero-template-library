...
import { ITemplateModel, ILateDeliveryAndPenaltyRequest } from './generated/io.clause.latedeliveryandpenaltyoptional@0.1.0';
...
describe('LateDeliveryAndPenaltyOptionalLogic', () => {
    ...
    beforeEach(() => {
        ...
        model = {
            $class: 'io.clause.latedeliveryandpenaltyoptional@0.1.0.TemplateModel',
            $identifier: 'test-id',
            clauseId: 'test-id',
            buyer: 'Alice',
            seller: 'Bob',
            forceMajeure: { $class: 'io.clause.latedeliveryandpenaltyoptional@0.1.0.Distance', miles: 100 },
    ...
    describe('trigger', () => {
        it('should return penalty=0 and buyerMayTerminate=true when both contract and request have force majeure set', async () => {
            const request: ILateDeliveryAndPenaltyRequest = {
                $class: 'io.clause.latedeliveryandpenaltyoptional@0.1.0.LateDeliveryAndPenaltyRequest',
                $timestamp: new Date(),
                forceMajeure: { $class: 'io.clause.latedeliveryandpenaltyoptional@0.1.0.Distance', miles: 50 },
        ...
        it('should emit a PaymentObligationEvent with correct fields', async () => {
            ...
            expect(event.$class).toBe('io.clause.latedeliveryandpenaltyoptional@0.1.0.PaymentObligationEvent');
