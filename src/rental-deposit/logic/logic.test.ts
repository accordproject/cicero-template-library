...
import { ITemplateModel, IProperyInspection } from './generated/org.accordproject.rentaldeposit@0.2.0';
...
describe('RentalDepositLogic', () => {
    ...
    describe('trigger', () => {
        it('should return full deposit when no penalties', async () => {
            ...
            expect(result.result.balance.currencyCode).toBe('USD');
        ...
        it('should emit a payment event', async () => {
                ...
                penalties: [
                    {
                        $class: `${NS}.Penalty`,
                        description: 'Damage',
                        amount: { $class: 'org.accordproject.money@0.3.0.MonetaryAmount', doubleValue: 500.0, currencyCode: 'USD' }
                    }
                ]
            };
            const result = await logic.trigger(model, request);
            ...
            expect(result.events).toHaveLength(1);
            expect((result.events[0] as any).amount.doubleValue).toBe(2000.0);
            expect((result.events[0] as any).amount.currencyCode).toBe('USD');
        });
    });
});
