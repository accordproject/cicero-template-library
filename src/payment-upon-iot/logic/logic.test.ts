...
} from "./generated/org.accordproject.paymentuponiot@0.3.0";

const NS = "org.accordproject.paymentuponiot@0.3.0";
...
describe("PaymentUponIoTLogic", () => {
    ...
    describe("trigger — unsupported request type", () => {
        it("should throw for an unknown $class", async () => {
            const state = runningState();
            const request = { $class: "org.accordproject.paymentuponiot@0.3.0.UnknownRequest", $identifier: "x" };
            await expect(logic.trigger(model, request, state))
