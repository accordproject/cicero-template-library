import { IFood, IOutcome, IBill, ITemplateModel } from "./generated/org.accordproject.eatapples@0.3.0";
...
class EatApplesLogic extends TemplateLogic<ITemplateModel> {
    async trigger(data: ITemplateModel, request: IFood): Promise<EatApplesResponse> {
        if (request.produce !== 'apple') {
            return {
                result: {
                    notice: "You're fired!",
                    $timestamp: new Date(),
                    $class: 'org.accordproject.eatapples@0.3.0.Outcome'
                },
                events: []
            };
        }
        ...
        const bill: IBill = {
            $class: 'org.accordproject.eatapples@0.3.0.Bill',
            $timestamp: new Date(),
