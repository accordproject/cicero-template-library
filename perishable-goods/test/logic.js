...
describe('Logic', () => {
    ...
    describe('#ShipmentReceived', async function() {
        ...
        it('on time delivery with min and max temp violations', async function() {
            ...
            const readingLow = {$class: `${NS}.TemperatureReading`, transactionId: 'a', shipment: 'SHIP_001', centigrade: 2};
            const readingOk = {$class: `${NS}.TemperatureReading`, transactionId: 'b', shipment: 'SHIP_001', centigrade: 5};
            const readingHigh = {$class: `${NS}.TemperatureReading`, transactionId: 'c', shipment: 'SHIP_001', centigrade: 15};            
            shipment.temperatureReadings = [readingLow, readingOk, readingHigh];
            request.shipment = shipment;
            const result = await engine.execute(clause, request);
            result.should.not.be.null;
            result.response.totalPrice.should.equal(3600);
            result.response.penalty.should.equal(900);
